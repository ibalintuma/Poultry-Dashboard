  ...
  <div class="card">
    ...
    <div class="" tabindex="-1" id="offcanvasAddUser" aria-labelledby="offcanvasAddUserLabel">
      <div class="offcanvas-header border-bottom">
        <h6 id="offcanvasAddUserLabel" class="offcanvas-title">Edit stocks</h6>
        <button type="button" class="btn-close text-reset" data-bs-dismiss="offcanvas" aria-label="Close"></button>
      </div>
      <div class="offcanvas-body mx-0 flex-grow-0">

        <form action='{{route("stocks.update", $obj->id)}}' method='post' enctype='multipart/form-data'>
          ...
          <div class='mb-3'>
            ...
            <select name='farm_id' class='form-control'>
              @foreach($farms as $f)
                <option value='{{$f->id}}'
                  @if($obj->farm_id == $f->id) selected @endif
                >{{$f->name}}</option>
              @endforeach
            </select>
          </div>
